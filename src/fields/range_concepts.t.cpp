...
struct optional_view_fn {
    ...
    {
        return optional_view(FWD(u), keep_bounds, rs::bind_back(*this, keep_bounds));
    }
...
TEST_CASE("optional_view")
{
    auto x = std::vector{1, 2, 3};
    auto y = std::vector{3, 4, 5, 6};

    auto x_false = x | vs::all | sel::optional_view(false);
    using x_false_t = decltype(x_false);

    REQUIRE(rs::size(x_false) == 0);
    REQUIRE(rs::random_access_range<x_false_t>);
    REQUIRE(rs::random_access_range<decltype(x_false.base())>);

    auto y_false = x_false.apply(y);
    REQUIRE(rs::size(y_false) == 0);
    REQUIRE(rs::random_access_range<decltype(y_false)>);

    auto x_true = x | vs::all | sel::optional_view(true);
    using x_true_t = decltype(x_true);

    REQUIRE(rs::size(x_true) == 3u);
    REQUIRE(rs::random_access_range<decltype(x_true.base())>);
    REQUIRE(rs::random_access_range<x_true_t>);
    REQUIRE(rs::contiguous_range<decltype(x_true.base())>);
    // this really should be true... should fix this at some point
    REQUIRE(!rs::contiguous_range<x_true_t>);
}
