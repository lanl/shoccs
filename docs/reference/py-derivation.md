...
| `conservation.py` | SBP conservation constraint build/solve with theta-linearization of bilinear terms: `build_conservation_system`, `solve_conservation`, `_interior_contribution`. |
...
3. **SBP conservation** (`solve_conservation`) — impose summation-by-parts column-sum conditions to fix quadrature weights `w_i` and resolve the last row's placeholder `phi_k`. The conservation equations are **bilinear** (`w_{r-1}*phi_k`); they are **linearized via theta-substitution** (`theta_k = w_{r-1}*phi_k`), solved linearly, then `phi_k = theta_k / w_{r-1}` recovered.
...
6. **Assembly** (`assemble_cut_cell_result`) → `CutCellResult` with `floating`/`dirichlet`/(`neumann`) matrices + SBP `weights`.
