...
Different idea: Since we need to keep a record of the points on the boundary anyway (i.e. $R^x$) we don't get a huge benefit from stuffing the points in $S^x$ and then pulling them out again. In this case we would write:

$$
\dfrac{\partial}{\partial x} U\bigg|_\mathcal{F} \approx O^x U\bigg|_\mathcal{F} + B^x U\bigg|_{\mathcal{R}^x}
$$

Floating (and Neumann) require:

$$
\begin{align*}
\dfrac{\partial}{\partial x} U\bigg|_{\mathcal{R}^x} & \approx\, ? \\
\dfrac{\partial}{\partial x} U\bigg|_{\mathcal{R}^y} & \approx\, ? \\
\dfrac{\partial}{\partial x} U\bigg|_{\mathcal{R}^z} & \approx\, ?
\end{align*}
$$
